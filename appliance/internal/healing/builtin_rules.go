...
func builtinRules() []*Rule {
	return []*Rule{
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "audit_logging"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
...

		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_ssh_config"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "LIN-SSH-001", "phases": []interface{}{"remediate", "verify"}},
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_kernel_params"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_cron_review"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_suid_binaries"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_firewall"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_audit_logging"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_failed_services"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "LIN-SVC-001", "phases": []interface{}{"remediate", "verify"}},
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_log_forwarding"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
		...
		},
		{
			...
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "windows_update"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "RB-WIN-SVC-001", "phases": []interface{}{"remediate", "verify"}},
		...
		// --- Missing Linux Check Types ---

		{
			ID:          "L1-LIN-DISK-001",
			Name:        "Linux Disk Space Critical",
			Description: "Linux disk usage above threshold — cleanup via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_disk_space"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "LIN-DISK-001", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.312(a)(1)"},
			Enabled:         true,
			Priority:        15,
			CooldownSeconds: 300,
		...
		},
		{
			...
			Description: "Unexpected open ports on Linux — escalate for human review",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_open_ports"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "Unexpected open ports require human investigation",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.312(e)(1)"},
			Enabled:         true,
		...
		{
			ID:          "L1-LIN-USERS-001",
			Name:        "Linux User Account Drift",
			Description: "Unauthorized user accounts on Linux — escalate for human review",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_user_accounts"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "User account changes require human verification",
				"urgency": "high",
			},
		...
		{
			ID:          "L1-LIN-UPGRADES-001",
			Name:        "Linux Unattended Upgrades Disabled",
			Description: "Automatic security updates not enabled — remediate via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_unattended_upgrades"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "LIN-PATCH-001", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.308(a)(5)(ii)(B)"},
			Enabled:         true,
			Priority:        10,
			CooldownSeconds: 300,
		...
		{
			ID:          "L1-LIN-CERT-001",
			Name:        "Linux Certificate Expiring",
			Description: "TLS certificate expiring on Linux — renew via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "linux_cert_expiry"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_linux_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "LIN-CERT-001", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.312(e)(1)"},
			Enabled:         true,
			Priority:        25,
			CooldownSeconds: 300,
		...
		// --- Missing Windows Check Types ---

		{
			ID:          "L1-WIN-DEFENDER-001",
			Name:        "Windows Defender Not Running",
			Description: "Windows Defender service stopped — restart via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "windows_defender"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "RB-WIN-SEC-003", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.308(a)(5)(ii)(B)"},
			Enabled:         true,
			Priority:        5,
			CooldownSeconds: 300,
		...
		{
			ID:          "L1-WIN-ROGUE-TASKS-001",
			Name:        "Rogue Scheduled Tasks Detected",
			Description: "Unauthorized scheduled tasks — escalate for human review",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "rogue_scheduled_tasks"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "Unauthorized scheduled tasks require human investigation",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.308(a)(5)(ii)(C)", "164.312(a)(1)"},
			Enabled:         true,
		...
		{
			ID:          "L1-WIN-AGENT-001",
			Name:        "Compliance Agent Not Running",
			Description: "Compliance monitoring agent stopped — restart via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "agent_status"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "RB-WIN-SVC-001", "phases": []interface{}{"remediate", "verify"}},
		...
		{
			ID:          "L1-WIN-PASSWD-001",
			Name:        "Windows Password Policy Drift",
			Description: "Password policy non-compliant — remediate via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "password_policy"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
		...
		{
			ID:          "L1-WIN-RDP-NLA-001",
			Name:        "Windows RDP NLA Disabled",
			Description: "RDP Network Level Authentication disabled — remediate via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "rdp_nla"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "RB-WIN-SEC-023", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.312(d)", "164.312(a)(1)"},
			Enabled:         true,
			Priority:        5,
			CooldownSeconds: 300,
		...
		},
		{
			...
			Description: "Guest account enabled — disable via runbook",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "guest_account"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action:          "run_windows_runbook",
			ActionParams:    map[string]interface{}{"runbook_id": "RB-WIN-SEC-024", "phases": []interface{}{"remediate", "verify"}},
			HIPAAControls:   []string{"164.312(a)(1)", "164.308(a)(3)(ii)(B)"},
			Enabled:         true,
			Priority:        5,
			CooldownSeconds: 300,
		...
		{
			ID:          "L1-NET-PORTS-001",
			Name:        "Network Unexpected Ports Open",
			Description: "Unexpected ports found open on network scan — escalate",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "net_unexpected_ports"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "Unexpected network ports require investigation",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.312(e)(1)"},
			Enabled:         true,
		...
		{
			ID:          "L1-NET-SVC-001",
			Name:        "Network Expected Service Missing",
			Description: "Expected network service not responding — escalate",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "net_expected_service"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "Expected network service not responding",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.312(a)(1)"},
			Enabled:         true,
		...
		{
			ID:          "L1-NET-REACH-001",
			Name:        "Network Host Unreachable",
			Description: "Expected host not reachable on network — escalate",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "net_host_reachability"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "Critical host unreachable on network",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.308(a)(7)(ii)(B)"},
			Enabled:         true,
			Priority:        5,
			CooldownSeconds: 300,
			MaxRetries:      1,
		...
		},
		{
			...
			Description: "DNS resolution failing on network — escalate",
			Conditions: []RuleCondition{
				{Field: "check_type", Operator: OpEquals, Value: "net_dns_resolution"},
				{Field: "drift_detected", Operator: OpEquals, Value: true},
			},
			Action: "escalate",
			ActionParams: map[string]interface{}{
				"reason":  "DNS resolution failing — potential network issue",
				"urgency": "high",
			},
			HIPAAControls:   []string{"164.312(e)(1)"},
			Enabled:         true,
