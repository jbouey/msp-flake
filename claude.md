...
- **Schema-fixture `prod_columns.json` does NOT surface NOT NULL / CHECK constraints — read the migration body, not the fixture (Session 220 #129 2-deploy-fail class, commits `7d9c33db` + `f606c7fe` + `ff7639d1`).** When introducing `INSERT INTO sites|users|client_orgs|site_appliances|compliance_bundles` or any table-touching DDL, fixture-presence of a column is necessary-but-not-sufficient. Mig 325 set `sites.client_org_id = NULL` (column exists in fixture so Gate A + Gate B passed) but prod has `NOT NULL` enforcement → CI deploy 500'd at apply time. Same class: mig 325 used `ON CONFLICT (key_hash)` for api_keys (column exists, no UNIQUE constraint exists). Forwarding rule: before any new INSERT, read the LATEST migration that creates/extends the target table + grep `prod_unique_indexes.json` + grep ALL prior migrations for `NOT NULL` / `CHECK` / `UNIQUE` on the relevant column. The fixture is a column-presence index, not a constraint index. Belt: when a NULL or duplicate would semantically be meaningful in your INSERT, prove it's *allowed* before assuming so.
