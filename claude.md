...
- **Backend-authoritative mesh (Session 196).** Target assignment is computed SERVER-SIDE in checkin (STEP 3.8c in sites.py). `hash_ring.py` must match Go's `mesh.go` exactly — cross-language test vectors at `appliance/internal/daemon/testdata/hash_ring_vectors.json`. Daemon prefers server assignments, falls back to local ring after 15 min. `mesh_topology` config and Network Stability UI are REMOVED.
...
- **`compliance_bundles` is the evidence table, NOT `evidence_bundles`.** evidence_bundles is a legacy table (1 row). compliance_bundles has 231K+ entries, Ed25519 signed, hash-chained, OTS-anchored.
