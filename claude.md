...
- **Time-travel reconciliation (Session 205, Phases 1/2/3).** Detects + recovers from VM snapshot revert / backup restore / disk clone. Migration 160 adds `reconcile_events` (append-only, RLS). Agent signals: `boot_counter_regression`, `uptime_cliff`, `generation_mismatch`, `lkg_future_mtime`. **`MIN_SIGNALS_REQUIRED=2` is wire-locked agent↔backend** — changing one side without the other rejects reconciles. Plan delivered INLINE in checkin response (`reconcile_plan` key), Ed25519-signed with same server key as orders.
- **Reconcile signature verification uses server's `signed_payload` string byte-exact (Session 205 Phase 3).** Do NOT reconstruct canonical JSON client-side — Python `json.dumps(sort_keys=True)` emits `", "` inside arrays but Go `json.Marshal` emits `","`. Reconstruction breaks verification. `appliance/internal/daemon/reconcile_apply.go` verifies against `plan.SignedPayload` directly. Freshness check uses `issued_at` extracted FROM signed payload (not envelope) to close replay window.
- **Reconcile admin-pool session intentional (Session 205).** `sites.py` inline reconcile opens `async_session()` (admin SQLAlchemy pool) not `tenant_connection()`. Mirrors the `/api/appliances/reconcile` endpoint's posture via `Depends(get_db)`. `auth_site_id` was enforced at checkin entry — do NOT flip to tenant_connection, it will break `reconcile_events` inserts. Documented at the import site in `sites.py`.
- **`Processor.PurgeAllNonces()` persists empty map (Session 205).** After reconcile nonce-epoch rotation, the daemon calls this — it clears the in-memory cache AND writes empty JSON to disk via `persistNoncesLocked`. A subsequent restart won't resurrect stale nonces.
- **Reconcile events admin UI at `/reconcile-events` (Session 205).** `ReconcileEvents.tsx`, admin-only, forensic timeline. Backend: `GET /api/admin/reconcile/events?site_id=...&limit=...` (reconcile.py `admin_router`, registered separately as `reconcile_admin_router` in main.py). User-facing text says "State Reconciliation" — NEVER "Time-Travel".
- **Migration auto-apply FAIL-CLOSED at startup (Session 205).** `main.py` lifespan calls `cmd_up()` from `migrate.py` before yielding. If any migration is pending or fails, `SystemExit(2)` — container refuses to serve. Rationale: the 2026-04-12 90-min fleet-order outage (missing migration 160 poisoned every checkin transaction, backend returned HTTP 200 while fleet orders silently starved). CI `deploy-central-command.yml` also runs `migrate.py status` + `up` + post-apply assertion with `set -e` (no more `|| echo` swallow).
- **`MIGRATION_DATABASE_URL` env var required on mcp-server container (Session 205).** Must point DIRECTLY to `mcp-postgres:5432` with `mcp` superuser (not `mcp_app` via PgBouncer) — `ensure_migrations_table` does `CREATE TABLE IF NOT EXISTS` which requires schema-public CREATE privilege that `mcp_app` lacks. Set in `/opt/mcp-server/docker-compose.yml` under `mcp-server.environment`. If missing, startup-apply fails with `InsufficientPrivilegeError: permission denied for schema public`.
- **`migrate.py cmd_up` takes `pg_advisory_lock(8675309)` (Session 205).** Serializes concurrent replicas at startup. Released in finally; connection close is belt-and-suspenders.
- **`/api/admin/health` schema probe (Session 205).** Returns `{"schema": {"applied": N, "pending": [...]}}`. Status goes `degraded` (HTTP 503) when `pending` is non-empty. Surfaces schema drift to operators before it causes silent outages.
- **Migration tracking: `schema_migrations` table (Session 205 backfill).** Holds one row per numbered migration (version + name + checksum + applied_at + execution_time_ms). Session 205 backfilled 152 rows for pre-existing files; future migrations record themselves via `INSERT` inside `apply_migration`. Also contains 5 legacy NAMED entries (`rls_tenant_isolation`, `flip_is_admin_default`, etc.) — these are untouched by backfill.
- **`sites.py` transactional step failures log at ERROR, not WARNING (Session 205).** 9 `logger.warning` → `logger.error` upgrades: steps 3.5 (appliances table), 3.5b (time-travel state), 3.6 (agent signing key), 3.6b (WireGuard), 3.8b (mesh peers), 3.9 (witness exchange), 4 (admin orders), 4.x (healing orders), 4.5 (fleet orders). Log shipper's alert rule fires on ERROR. Silent savepoint failures that caused the 2026-04-12 outage would page ops on the first cycle under this change.
- **`nix.gc.automatic = true` + `nix.optimise.automatic = true` required (Session 205).** `/etc/nixos/configuration.nix` on the VPS. Prior to Session 205 there was no nix-gc timer and the store hit 98G (150G disk filled, postgres crashed). Weekly cadence, deletes generations older than 14d, `persistent = true` catches missed runs after reboot.
- **asyncpg savepoint invariant (Session 205 hardening round-table).** Every `conn.execute/conn.fetch/conn.fetchrow/conn.fetchval` inside `async with tenant_connection()` or `async with admin_connection()` whose failure is caught non-fatally MUST be wrapped in `async with conn.transaction():` (savepoint). One unprotected failing statement poisons every subsequent statement on the same connection — the exception you caught in Python does NOT reset the Postgres transaction state. This caused the 2026-04-12 90-min fleet-order delivery outage: `get_fleet_orders_for_appliance` had two `DELETE FROM fleet_order_completions` inside `try/except: pass` without savepoints, a DELETE trigger blocked them, outer transaction aborted, STEP 4.5 returned no orders, every checkin silently starved for orders. Applies to SQLAlchemy `async with db.begin_nested():` savepoints analogously.
- **No silent write failures (Session 205 hardening round-table).** `except Exception: pass` on a code path containing a DB write is BANNED. `logger.warning` on DB failures is BANNED — use `logger.error(exc_info=True)` so the log shipper alert rule fires. Writes that genuinely can be lost log-then-raise; reads can eat exceptions.
- **Audit-trigger allowlist (Session 205 hardening round-table).** DELETE/UPDATE-block triggers apply ONLY to tables that genuinely hold audit records. The 2026-04-12 outage was caused by Migration 151 applying `prevent_audit_deletion` to `fleet_order_completions` — an OPERATIONAL state table (delivery tracking, needs UPDATE/DELETE for retry + regression). Taxonomy: audit (admin_audit_log, client_audit_log, portal_access_log, incident_remediation_steps, partner_activity_log), evidence (compliance_bundles — requires special handling per next invariant), operational (fleet_order_completions, fleet_orders, sessions, nonces, discovered_devices, go_agents), config (sites, appliances, organizations, partners, users), transient (mfa_pending_tokens, oauth_state). Migration 161 dropped the overreaching trigger from fleet_order_completions; future migrations adding DELETE-block triggers require a `table_classifications` entry + `test_audit_trigger_allowlist.py` passes.
- **Evidence bundles are immutable — no DELETE upsert (Session 205 hardening).** `compliance_bundles` has a `prevent_audit_deletion` BEFORE DELETE trigger (correct policy). Attempting to DELETE-then-INSERT as "upsert" violates HIPAA audit immutability and silently fails in production. `evidence_chain.py submit_evidence` (line ~1156) checks bundle_id existence FIRST and returns idempotent-dedup if present. ON CONFLICT is incompatible with partitioned tables, so the check-then-insert pattern is the correct shape.
- **Checkin is a delivery contract (Session 205 hardening round-table).** Returning HTTP 200 while silently failing fleet-order delivery is a credibility-grade event for a compliance-attestation platform. Observability floor: Prometheus gauge `msp_fleet_active_orders_undelivered{appliance_id}` alerts on value > 0 for > 15 min. Log shipper alerts on any `logger.error` containing "transaction" / "savepoint" / "cleanup" / "Failed to fetch". Any PR that makes a checkin transactional-step failure quieter than ERROR is a rollback.
- **CI must run full integration round-trip before merge to main (Session 205 hardening round-table).** Ephemeral postgres (via testcontainers) + `migrate.py up` + `uvicorn main:app` + mocked appliance POST to `/api/appliances/checkin` + assert response contains a pre-inserted active fleet_order. This one test catches the ENTIRE failure class that caused the 2026-04-12 outage: import errors (module loads), migration drift (DDL applies), checkin-contract regressions (order actually arrives). Shape tests alone ("regex check for `sort_keys=True`" etc.) are theater.
