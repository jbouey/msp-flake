...
- **Fleet order health check rollback bug (Session 204).** The `update_daemon` handler's goroutine (health check + rollback) dies when `systemctl restart` kills the old daemon. The goroutine runs in the OLD process. First appliance may succeed; others may silently rollback. Manual binary swap via WireGuard as workaround.
