...
- **PREREQ_SCHEMA DROP/CREATE pairing (Session 220 2026-05-16).** Any `CREATE TABLE` added to a pg test fixture's `PREREQ_SCHEMA` (e.g. `tests/test_startup_invariants_pg.py`) MUST have a matching `DROP TABLE IF EXISTS <name> CASCADE;` earlier in the same string. Otherwise test #2 in the same run hits `DuplicateTableError` because the fixture is per-test (drops everything, then creates everything). Local sweep cannot catch — `*_pg.py` files are SKIPPED locally (asyncpg dep), only run in CI with a real PG sidecar. Burned 1 CI deploy cycle in `74226abc`→`71233389`. When extending a PREREQ_SCHEMA, grep for the existing DROP block + add the new line there FIRST, then the CREATE.
- **F-string-interpolated table aliases break SQL-column gates (Session 220 2026-05-16).** Substrate invariants that iterate per-table (`for tbl in _SOME_LIST: conn.fetch(f"SELECT t.col FROM {tbl} t ...")`) had a silent gap in `tests/test_substrate_invariant_sql_columns_valid.py` — the alias resolver didn't see `FROM {var} alias` patterns + treated `t.col` refs as unresolvable → silent skip. Same bug class shipped twice (C5a-rev1 `_check_synthetic_traffic_marker_orphan` + iter-4 Commit 2). Hardening in `74226abc`: regex matches f-string patterns + validates the column against EVERY iteration target (pessimistic universal check). When adding a per-table loop with f-string SQL, ensure the iterated tables ALL have the columns the SQL references; the gate will catch you if any don't.
...
- **PYTHON_ONLY vs GO_ONLY decision rule (Session 220 #123 Sub-A Gate B P0 fix, commit `5106bf19`).** When registering a new privileged order type, ask: *does the daemon RECEIVE this order from the fleet_orders queue + DISPATCH it to a local handler?* If YES → register in all 4 lists including Go `appliance/internal/orders/processor.go::dangerousOrderTypes` (signature-verification gate) AND ship a matching Go handler (or `TestDangerousHandlersRegistered` fails). If NO (server-side-only — handler is a Central Command DB UPDATE or admin API call) → register in 3 lists ONLY (`fleet_cli.PRIVILEGED_ORDER_TYPES` + `ALLOWED_EVENTS` + mig `v_privileged_types`) AND add to `tests/test_privileged_order_four_list_lockstep.py::PYTHON_ONLY` allowlist with rationale. The Go map is for daemon-execution gating, not for "defense-in-depth" decoration — adding an entry without a handler IS the regression (silent dead-list class that round-table-2026-04-29 P3 closed via the configure_workstation_agent removal). Examples: `delegate_signing_key` (Python-only — backend mints the key, daemon requests via POST), `bulk_bearer_revoke` (Python-only — backend UPDATE on `site_appliances.bearer_revoked`, daemon's next checkin hits 401 via `shared.py:614-640` short-circuit). CI deploy workflow does NOT run Go tests — local `go test ./appliance/internal/orders/...` is the only gate. Pinned by `TestDangerousHandlersRegistered` + the PYTHON_ONLY allowlist test.
...
- **Schema-fixture `prod_columns.json` does NOT surface NOT NULL / CHECK constraints — read the migration body, not the fixture (Session 220 #129 3-deploy-fail class, commits `7d9c33db` + `f606c7fe` + `ff7639d1` + `7b7fafaf`).** When introducing `INSERT INTO sites|users|client_orgs|site_appliances|compliance_bundles|api_keys` or any table-touching DDL, fixture-presence of a column is necessary-but-not-sufficient. Three consecutive failure modes shipped same task: (1) `ON CONFLICT (api_keys.key_hash)` — column exists, no UNIQUE constraint exists; (2) `sites.client_org_id = NULL` — column exists, NOT NULL enforced; (3) `api_keys.key_prefix = 'load-test-bearer-NN-prefix'` (26 chars) — column exists with varchar(16) limit. Forwarding rule before any new INSERT, in ORDER: (a) read the LATEST migration that creates/extends the target table (look at the CREATE TABLE / ALTER TABLE body for NOT NULL / CHECK / FK); (b) grep `prod_unique_indexes.json` for the table (UNIQUE constraints constrain `ON CONFLICT` targets); (c) grep `prod_column_widths.json` for the table (varchar widths constrain INSERT values); (d) grep `prod_column_types.json` for the table (type matters for cast safety). All four fixtures together are the constraint envelope; ANY ONE missing means you ship a deploy-fail. Belt: when a NULL, duplicate, OR oversize value would semantically be meaningful in your INSERT, prove it's *allowed* before assuming so.
