...
`SIGNING_BACKEND` env var) disagrees with the observed
`fleet_orders.signing_method` distribution in the last hour.
Mismatch = chain-of-custody drift on the production signing path.

The invariant compares:
- **Expected**: env-derived. `SIGNING_BACKEND` directly, unless env=
  `shadow`, in which case `SIGNING_BACKEND_PRIMARY`.
...
1. **Silent fallback to disk-key signing.** Vault unreachable + the
   ...
   contains `file`.
2. **Container env desynced from running state.** Partial restart or
   `docker compose up -d` without env re-source picked up stale values.
3. **INSERT call-site missed `current_signing_method()`.** A new
   fleet_orders INSERT was added without the column write. P0 #3
   regression (defaults to column DEFAULT 'file').
4. **Pre-cutover bootstrap.** During shadow mode, env may declare
   `shadow` while the PRIMARY is `file` — the invariant carves this
   out (compares against `SIGNING_BACKEND_PRIMARY` when env=`shadow`).
...
4. If env declares `file` but observed contains `vault`: grep recent
   commits for INSERT INTO fleet_orders to find the path that wrote
   'vault' when it shouldn't have.

## Verification

After remediation:
- Trigger a benign fleet_order.
...
- `INV-SIGNING-BACKEND-VAULT` (startup_invariants.py) — sibling
  startup-time check for key-version pinning. Bound by `asyncio.wait_for(5s)`
  so it never blocks container boot.
- `bg_loop_silent` — if Vault host's WireGuard heartbeat is dead, the
  signing-backend drift is downstream of network availability.

## Change log

- 2026-05-13: Re-created after Vault Phase C P0 revert chain
  (`9fa26a54..686a9b76`). Iter-3 root cause (startup hang) closes via
  `asyncio.wait_for(5s)` + lifespan eager-warm. Pre-cutover posture:
  env=`shadow` PRIMARY=`file` — invariant idle as long as every
  fleet_order signs via `file`.
