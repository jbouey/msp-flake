...
**COMBINED REGEN (Task #77 Phase B-lite + #87, P0-D1)** — all THREE
schema sidecars (`prod_columns.json`, `prod_column_types.json`,
`prod_column_widths.json`) are regenerated by ONE prod query so they
can never drift to different snapshots. The query pulls
`[data_type, character_maximum_length]` per column; names/types/widths
are all derived locally. Key-set parity is pinned by
`test_schema_fixture_parity.py` (names↔types identical; widths ⊆ names).
Run from `backend/`:

    ssh root@178.156.162.116 "docker exec mcp-postgres psql -U mcp -d mcp -t -A -c \"
        SELECT json_object_agg(table_name, cols) FROM (
            SELECT table_name, json_object_agg(column_name, json_build_array(data_type, character_maximum_length)) AS cols
    ...
    \"" | python3 -c "
        ...
        typed, names, widths = {}, {}, {}
        for t, cols in sorted(d.items()):
            typed[t] = {c: v[0] for c, v in sorted(cols.items())}
            names[t] = sorted(cols.keys())
            w = {c: v[1] for c, v in sorted(cols.items()) if v[1] is not None}
            if w:
                widths[t] = w
        for fn, obj in [('prod_column_types.json', typed), ('prod_columns.json', names), ('prod_column_widths.json', widths)]:
...
(a) From a clean prod (preferred — no manual edits to the JSON): run
the combined command above.

(b) Forward-merge a local migration before it deploys (lockstep with
the migrations/N_*.sql files in the same PR). Edit all THREE fixtures in
the same diff (keeping names↔types parity and widths ⊆ names); CI deploy
will run the migration and the post-deploy fixture will match. This is
what the 2026-04-25 baseline-grind pass did with migrations 248 + 249.
...
# baseline 12 → 11.
SELECT_BASELINE_MAX = 10  # Session 220 task #120 PR-A 2026-05-12: 11→9 after deleting report_discovery_results + verify_ots_bitcoin SQL bodies. 2026-05-14 (#77 Phase B-lite): 9→10 — fixture refresh made client_org_owner_transfer_requests newly-checkable; client_owner_transfer.py:557 flagged is walker imprecision (the real query references only id/client_org_id/initiated_by_user_id/target_email/reason/expires_at/status, all valid — flagged tokens are variable names + other-statement columns), same false-positive class as the existing baseline entries.
