...
**COMBINED REGEN (Task #77 Phase B-lite, P0-D1)** — `prod_columns.json`
and its typed sidecar `prod_column_types.json` are regenerated by ONE
command so they can never drift. The names fixture is DERIVED from the
typed pull, so key-set parity is guaranteed by construction (and pinned
by `test_schema_fixture_parity.py`). Run from `backend/`:

    ssh root@178.156.162.116 "docker exec mcp-postgres psql -U mcp -d mcp -t -A -c \"
        SELECT json_object_agg(table_name, cols) FROM (
            SELECT table_name, json_object_agg(column_name, data_type) AS cols
    ...
    \"" | python3 -c "
        ...
        typed = {k: dict(sorted(v.items())) for k, v in sorted(d.items())}
        names = {k: sorted(v.keys()) for k, v in typed.items()}
        for fn, obj in [('prod_column_types.json', typed), ('prod_columns.json', names)]:
...
the migrations/N_*.sql files in the same PR). Edit BOTH fixtures in the
same diff (keeping key-set parity); CI deploy will run the migration
and the post-deploy fixture will match. This is what the 2026-04-25
baseline-grind pass did with migrations 248 + 249.
