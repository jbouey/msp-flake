...
BASELINE_MAX = 0  # 2026-05-16 Fork B leaks CLOSED via canonical primitive extraction. compliance_score.compute_category_weighted_overall(cat_pass, cat_fail, cat_warn, *, category_weights, partial_credit_warning=0.5) shipped — all 4 category_weighted_compliance_score callsites delegate (db_queries.get_compliance_scores_for_site + get_all_compliance_scores + routes.get_admin_compliance_health + client_portal.get_site_compliance_health). All 4 reclassified `operator_only` (each implements distinct upstream bundle-scan semantics but shared per-category formula). Counsel Rule 1 satisfied: one canonical source for the formula. Fork A spec §2 approach (b) shipped as minimal first pass; full canonical helper extension (period-bounded, etc.) remains Class-B Gate A future work.
