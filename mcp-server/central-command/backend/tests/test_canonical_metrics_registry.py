...
BASELINE_MAX = 0  # **Phase 3 FULLY CLOSED 2026-05-15 (Task #103). All 26 migrate-class entries resolved**: 7 reclassified as `operator_only` after per-entry methodology inspection revealed distinct metric classes (compliance_score slice 6 + baa_on_file slice 1) — with 3 new PLANNED_METRICS entries registering the un-canonicalized classes (historical_period_compliance_score, category_weighted_compliance_score, per_framework_compliance_score). 19 device_count_per_site line-anchored entries REMOVED after verification showed 4 already-migrated-with-marker, 1 uses-canonical-CTE, 14 stale-lines (post-BUG-1-drive-down). Device-count canonical migration tracking now lives in test_no_raw_discovered_devices_count.py BASELINE_MAX (=0 per Task #74 close-out 2026-05-15) + per-line `canonical-migration:` markers. Any future canonical-source drift surfaces via the sibling ratchet, not via re-adding line-anchored entries here. Counsel Rule 1 enforcement is fully gated at registry-integrity layer.
