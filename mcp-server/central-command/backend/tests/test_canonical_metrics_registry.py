...
BASELINE_MAX = 2  # 2026-05-16 Fork B post-Phase-3 leak audit reopened the count: routes.get_admin_compliance_health + client_portal.get_site_compliance_health both compute category_weighted_compliance_score INLINE (per-category `(pass + 0.5*warn)/total*100` with HIPAA_CATEGORY_WEIGHTS) instead of delegating to db_queries.get_compliance_scores_for_site. category_weighted_compliance_score was promoted from PLANNED_METRICS to CANONICAL_METRICS in lockstep with this registration. Long-term solution = Fork A's compute_category_weighted_score helper spec (audit/coach-103-canonical-helper-extension-designs-2026-05-16.md §2) — proper extraction of the per-category formula into a shared primitive that all 4 callsites can delegate to. Until that Class-B Gate A completes + ships, the 2 leak callsites are gated migrate-class entries with stable function-name anchors (post the line-anchored-fragility lesson from Task #74).
