...
const navItems: NavItem[] = [
  ...
  {
    // Substrate Health is the "what's broken NOW" view — placed above
    // the runbook entries because operator priority on the admin
    // section is current incidents > playbooks. Authz: backend uses
    // require_auth (any logged-in user); we match here — no adminOnly,
    // since the substrate engine's purpose is operator visibility
    // into substrate state.
    path: '/admin/substrate-health',
    label: 'Substrate Health',
    section: 'admin',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        {/* Heart-pulse: substrate integrity / vital signs */}
  ...
  {
    // Label disambiguates from the L1/L2 Runbooks entry above.
    // Authz: backend require_auth — match it (no adminOnly).
    path: '/admin/substrate/runbooks',
    label: 'Substrate Runbook Library',
    section: 'admin',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        {/* Document-list: invariant runbook library */}
