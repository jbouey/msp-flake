...
export const ClientEvidence: React.FC = () => {
  ...
  const getResultBadge = (result: string) => {
    switch (result) {
      ...
      default:
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700">{result}</span>;
    }
  };
    ...
    <div className="min-h-screen bg-slate-50/80 page-enter">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-semibold text-slate-900">Evidence Archive</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <span className="text-sm text-slate-500">{total} total bundles</span>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-slate-500">No evidence bundles found</div>
          ) : (
          ...
          {total > limit && (
            <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
              <button
                ...
                disabled={offset === 0}
                className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-teal-50/50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
