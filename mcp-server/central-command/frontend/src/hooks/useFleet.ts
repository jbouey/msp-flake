...
export interface FlywheelSpine {
  ...
  self_heal_rate_24h_pct: number | null;
  self_heal_rate_7d_pct: number | null;
  self_heal_24h: { total: number; l1: number; l2: number; l3: number; pct: number | null };
  self_heal_7d: { total: number; l1: number; l2: number; l3: number; pct: number | null };
  trend_7d: Array<{ date: string | null; total: number; l1: number; pct: number | null }>;
  per_site_24h: Array<{ site_id: string; total: number; l1: number; pct: number | null }>;
...
export function useOrders(siteId: string | null, status?: string) {
  ...
  return useQuery<OrderResponse[]>({
    ...
    queryFn: () => ordersApi.getOrders(siteId!, status as OrderStatus | undefined),
    enabled: !!siteId,
    ...defaults,
  });
}
...
export function useIncidentBreakdown(window: '24h' | '7d' | '30d' = '24h', siteId?: string) {
  const defaults = useQueryDefaults();
  return useQuery<IncidentBreakdownResponse>({
    queryKey: ['incident-breakdown', window, siteId],
    queryFn: () => commandCenterApi.getIncidentBreakdown(window, siteId),
    ...defaults,
  });
}
